...
- **주횟수 할인율(%)** — 요금표에서 유형별 `% 할인율`로 주2~5회 요금을 자동 계산(주1회가 × 횟수 × (1−할인율)), 구분별 주1회 금액에 각각 적용·저장
...
- **Per-frequency discount %** — in the price table, fill 2–5×/week prices from a `% discount` per lesson type (1×/week base × N × (1−discount)), applied to each category's own 1×/week price and saved
