...
- **기본 결제 개월** — ⚙️ 설정에서 신규 회원 등록 시 기본 선택되는 결제 개월(2달치/3달치)을 지정
- **주횟수 할인율(%)** — 요금표에서 유형별 `% 할인율`로 주2~5회 요금을 자동 계산(주1회가 × 횟수 × (1−할인율)). **통합이면 공통 1세트, 분리면 성인·청소년·유소년별로 각각 다른 할인율** 지정 가능(저장·다음에 프리필)
...
- **Default payment term** — set the term (2 or 3 months) pre-selected when registering a new member, in ⚙️ Settings
- **Per-frequency discount %** — in the price table, fill 2–5×/week prices from a `% discount` (1×/week base × N × (1−discount)); **one shared set when unified, or a separate discount per category (adult/teen/youth) when split**, applied to each category's own 1×/week price and saved
